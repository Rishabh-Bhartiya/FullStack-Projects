...
function Result() {
  ...
  return (
    <motion.form
      ...
      className='flex flex-col min-h-[90vh] justify-center items-center text-center'
    >
      <div className='relative w-full max-w-[280px] sm:max-w-xs md:max-w-sm lg:max-w-md'> {/* Drastically reduced max-width */}
      ...
      {!isImageLoaded && (
        <div className='flex w-full max-w-xl bg-zinc-900 border border-zinc-800 text-gray-200 p-1 mt-10 rounded-full shadow-lg'>
          <input
            onChange={(e) => setInput(e.target.value)}
            value={input}
            type="text"
            placeholder='Describe what you want to generate...'
            className='placeholder:text-gray-500 flex-1 bg-transparent outline-none ml-6 mr-2'
          />
          <button
            type='submit'
            className='bg-blue-600 hover:bg-blue-700 px-8 sm:px-16 py-3 rounded-full transition-colors duration-300'
          >
            Generate
          </button>
        </div>
